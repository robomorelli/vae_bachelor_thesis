...
\begin{align*} 
	KL (q(z|x) || p(z|x)) &=  -\sum q(z|x) \log \frac{p(x|z)p(z)}{p(x)q(z|x)} \\ &=  -\sum q(z|x)\log \frac{p(x|z)p(z)}{q(z|x)} + \sum q(z|x)\log p(x)
\end{align*}
ma in entrambi i termini della somma la sommatoria è estesa sui vari z
\newpage