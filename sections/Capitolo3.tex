...
Dall'osservazione qualitativa della figura~\ref{ricostruzione} emerge che il processo di ricostruzione del VAE risulta essere molto accurato per tutte le variabili, ad eccezione della $\textit{mbb}$. \\
Come detto, la sola variabile con ricostruzione non soddisfacente è la $\textit{mbb}$, tuttavia, come si vedrà più avanti, è possibile correggere questo risultato impostando un peso maggiore per tale variabile (ad esempio un peso pari a due o tre rispetto agli tutti pari ad uno). \\
...
Come noto, l'errore per ogni singolo evento viene calcolato sommando quello sulle otto variabili (differenza fra punto blu e rosso) e, da qui, si ottiene la distribuzione della $\textit{Loss}$, come quella riportata in figura \ref{distribuzione_loss}; in questa immagine, oltre alla distribuzione relativa agli eventi di background ( per il data set di validation e di test), sono presenti anche le distribuzione relative ad alcune delle ipotesi di segnale contraddistinte dalla diverse ipotesi sulle masse delle due particelle (chargino/gluino). E' importante ricordare che le simulazioni MC degli eventi di segnale non sono mai state usate fino ad ora e solo in questa fase di test vengono date in input al modello. \\
